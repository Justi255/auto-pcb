...
float BoardGrid::get2dMultiTargetEstimatedCost(const Location &l) {
    ...
    for (const auto &target : this->currentTargetedPinWithLayers) {
        ...
        cost = std::min(cost, (float)(GlobalParam_router::gDiagonalCost * minDiff + maxDiff - minDiff));
    }
    return cost;
}
...
float BoardGrid::get2dEstimatedCost(const Location &l) {
    ...
    int minDiff = min(absDiffX, absDiffY);
    int maxDiff = max(absDiffX, absDiffY);
    return (float)minDiff * GlobalParam_router::gDiagonalCost + maxDiff - minDiff;
}
...
float BoardGrid::get2dEstimatedCostWithBendingCost(const Location &current, const Location &next) {
    ...
    // int minDiff = min(absDiffX, absDiffY);
    // int maxDiff = max(absDiffX, absDiffY);
    // return (float)minDiff * GlobalParam_router::gDiagonalCost + maxDiff - minDiff - bendingCost;
...
float BoardGrid::get3dEstimatedCost(const Location &l) {
    ...
    int minDiff = min(absDiffX, absDiffY);
    int maxDiff = max(absDiffX, absDiffY);
    float estCost = (float)minDiff * GlobalParam_router::gDiagonalCost + maxDiff - minDiff;

    // If is SMD pin, add the layer changing cost
    if (currentTargetedPinWithLayers.size() == 1) {
        estCost += GlobalParam_router::gLayerChangeCost * abs(this->currentTargetedPinWithLayers.front().m_z - l.m_z);
    }
    return estCost;
}
...
float BoardGrid::get3dEstimatedCostWithBendingCost(const Location &current, const Location &next) {
    ...
    int minDiff = min(absDiffX, absDiffY);
    int maxDiff = max(absDiffX, absDiffY);
    float estCost = (float)minDiff * GlobalParam_router::gDiagonalCost + maxDiff - minDiff - bendingCost;

    // If is SMD pin, add the layer changing cost
    if (currentTargetedPinWithLayers.size() == 1) {
        estCost += GlobalParam_router::gLayerChangeCost * abs(this->currentTargetedPinWithLayers.front().m_z - next.m_z);
    }
    return estCost;
}
...
void BoardGrid::getNeighbors(const Location &l, std::vector<std::pair<float, Location>> &ns) {
    ...
    if (GlobalParam_router::gAllowViaForRouting) {
        if (GlobalParam_router::gUseMircoVia) {
            // up
            if (l.m_z + 1 < this->l) {
                ...
                upCost += GlobalParam_router::gLayerChangeCost;
            ...
            if (l.m_z - 1 > -1) {
                ...
                downCost += GlobalParam_router::gLayerChangeCost;
            ...
            //     viaCost += GlobalParam_router::gLayerChangeCost;
            ...
            if (this->cached_via_cost_at(viaCachedLocation) < -1.5) {
                ...
                if (this->cached_via_cost_at(viaCachedLocation) < -0.5) {
                    ...
                    viaCost = this->cached_via_cost_at(viaCachedLocation) + GlobalParam_router::gLayerChangeCost;
    ...
    */
    ...
    if (l.m_x - 1 > -1 && l.m_y + 1 < this->h) {
        ...
        float lfCost = GlobalParam_router::gDiagonalCost;
        ...
        if (this->cached_trace_cost_at(lf) == -1) {
            ...
            // Put in cache
            this->cached_trace_cost_set(lfCost - GlobalParam_router::gDiagonalCost, lf);
        } else {
    ...
    if (l.m_x - 1 > -1 && l.m_y - 1 > -1) {
        ...
        float lbCost = GlobalParam_router::gDiagonalCost;
        ...
        if (this->cached_trace_cost_at(lb) == -1) {
            ...
            // Put in cache
            this->cached_trace_cost_set(lbCost - GlobalParam_router::gDiagonalCost, lb);
        } else {
    ...
    if (l.m_x + 1 < this->w && l.m_y + 1 < this->h) {
        ...
        float rfCost = GlobalParam_router::gDiagonalCost;
        ...
        if (this->cached_trace_cost_at(rf) == -1) {
            ...
            // Put in cache
            this->cached_trace_cost_set(rfCost - GlobalParam_router::gDiagonalCost, rf);
        } else {
    ...
    if (l.m_x + 1 < this->w && l.m_y - 1 > -1) {
        ...
        float rbCost = GlobalParam_router::gDiagonalCost;
        ...
        if (this->cached_trace_cost_at(rb) == -1) {
            ...
            // Put in cache
            this->cached_trace_cost_set(rbCost - GlobalParam_router::gDiagonalCost, rb);
        } else {
...
void BoardGrid::printGnuPlot() {
    ...
    for (int l = 0; l < this->l; ++l) {
        ...
        outFileName =
            util_router::appendDirectory(GlobalParam_router::gOutputFolder, outFileName);
        ...
        for (int y = -radius; y <= radius; ++y) {
            for (int x = -radius; x <= radius; ++x) {
                ...
                if (!validate_location(current_l)) {
                    cost += GlobalParam_router::gViaTouchBoundaryCost;
                    continue;
                }
...
bool BoardGrid::sizedViaExpandableAndCost(const Location &l, const std::vector<Point_2D<int>> &viaRelativeSearchGrids, float &cost) const {
    ...
    for (int z = 0; z < this->l; ++z) {
        for (const auto &gridPt : viaRelativeSearchGrids) {
            ...
            if (this->isViaForbidden(current_l)) {
                //return false;
                cost += GlobalParam_router::gViaForbiddenCost;
                continue;
            }
            //cost += this->via_cost_at(current_l);
            cost += this->base_cost_at(current_l);
...
float BoardGrid::sized_trace_cost_at(const Location &l, const std::vector<Point_2D<int>> &traRelativeSearchGrids) const {
    ...
    for (const auto &gridPt : traRelativeSearchGrids) {
        ...
        if (!validate_location(current_l)) {
            cost += GlobalParam_router::gTraceTouchBoundaryCost;
            continue;
        }
        cost += this->base_cost_at(current_l);
        // cost += this->via_cost_at(current_l);
...
void BoardGrid::remove_route_from_base_cost(const MultipinRoute &route) {
    ...
    // Old API below
    // add_route_to_base_cost(route, traceExpandingRadius, -GlobalParam_router::gTraceBasicCost, viaExpandingRadius, -GlobalParam_router::gViaInsertionCost);
...
void BoardGrid::getCostsVecByRadius(const float centerCost, const int radius, vector<float> &costVec) {
    ...
    if (!GlobalParam_router::gCurvingObstacleCost) return;

    for (int i = 0; i <= radius; ++i) {
        costVec.at(i) = centerCost - (centerCost * pow((double)i / (double)radius, GlobalParam_router::gObstacleCurveParam));
    }
}
    ...
    for (; nextPointIte != segs.end();) {
        ...
        if (pointIte->x() == nextPointIte->x() && pointIte->y() != nextPointIte->y()) {
            ...
            for (auto curY = startY; curY <= endY; ++curY) {
                ...
                //this->add_pseudocost(curX + curY * this->w + curZ * this->w * this->h, 1.0);
            }
            // Extended Line
        ...
        // LL -> UR || UR -> LL
            ...
            for (int curX = startX, curY = startY; curX <= endX && curY <= endY; ++curX, ++curY) {
                this->base_cost_add(traceCost, Location(curX, curY, curZ));
                //this->add_pseudocost(curX + curY * this->w + curZ * this->w * this->h, 2.0);
            }
            // Extended Line
...
void BoardGrid::backtrackingToGridPath(const Location &end, MultipinRoute &route) const {
    ...
    int currentId = this->locationToId(current);
    int nextId = this->getCameFromId(currentId);
    //double cost = 1.0 + this->grid[currentId].bendingCost;
    //this->add_pseudocost(currentId,cost);
    ...
    while (nextId != -1) {
        ...
        //cost = 1.0 + this->grid[nextId].bendingCost;
        //this->add_pseudocost(nextId, cost);
    }
...
void BoardGrid::addPinShapeObstacleCostToGrid(const GridPin &gridPin, const float value, const bool toViaCost, const bool toViaForbidden, const bool toBaseCost) {
    ...
    if (GlobalParam_router::gVerboseLevel <= VerboseLevel::NOTSET) {
        //std::cout << __FUNCTION__ << "()"
...
void BoardGrid::convertDiffPairPathToTwoNetPaths(GridDiffPairNet &route) {
    ...
    int traceDiagonalOffset = (int)ceil(GlobalParam_router::gTan22_5 * traceClearance);
...
void BoardGrid::routeGridDiffPairNet(GridDiffPairNet &route) {
    ...
    bool allowViaForRouting = GlobalParam_router::gAllowViaForRouting;
    GlobalParam_router::gAllowViaForRouting = false;
    ...
    // Put back default setting
    GlobalParam_router::gAllowViaForRouting = allowViaForRouting;
...
void BoardGrid::routeGridNetWithRoutedGridPaths(MultipinRoute &route, const bool removeGridPinObstacles, const bool routedPathsToGridCost) {
    ...
    if (removeGridPinObstacles) {
        this->addPinShapeObstacleCostToGrid(route.mGridPins, -GlobalParam_router::gPinObstacleCost, true, false, true);
    }
...
void BoardGrid::add_pseudocost(const int id, float num) const{
    this->grid[id].pseudo_cost += num;
}

double BoardGrid::count_allpseudo() const{
    double all_cost = 0;
    for(int i = 0; i < this->size; ++i){
        all_cost += exp(this->grid[i].pseudo_cost);
    }
    //std::cout << "Pseudo_cost is :: " << all_cost;
    return all_cost;
}