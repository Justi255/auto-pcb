...
at::Tensor pin_pos_forward(at::Tensor pos, at::Tensor pin_offset_x,
  ...
  DREAMPLACE_DISPATCH_FLOATING_TYPES(pos, "computePinPosLauncher", [&] {
    computePinPosLauncher<scalar_t>(
        ...
        1, DREAMPLACE_TENSOR_DATA_PTR(out, scalar_t),
...
#pragma omp parallel for num_threads(num_threads)
  for (int i = 0; i < num_pins; ++i) {
    ...
    // if(std::isnan(pin_x[i])||std::isinf(pin_x[i])||std::isnan(pin_y[i])||std::isinf(pin_y[i])){
    //   std::cout<<" node_id: "<<node_id<<" x: "<<x[node_id]<<" y: "<<y[node_id]<<" i: "<<i<<" pin_x[i]: "<<pin_x[i]<<" pin_y[i]: "<<pin_y[i]<<std::endl;
    //   std::cout<<" pin_offset_x[i]: "<<pin_offset_x[i]<<" pin_offset_y[i]: "<<pin_offset_y[i]<<std::endl;
    // }
  }
