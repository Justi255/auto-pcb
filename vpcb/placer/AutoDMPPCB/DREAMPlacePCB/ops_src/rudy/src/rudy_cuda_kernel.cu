...
__global__ void rudy(const T *pin_pos_x,
                              ...
                              typename AtomicOp::type *utilization_map)
{
    ...
    if (i < num_nets)
    {
        ...
        for (int x = bin_index_xl; x < bin_index_xh; ++x)
        {
            for (int y = bin_index_yl; y < bin_index_yh; ++y)
            {
                ...
                atomic_add_op(&utilization_map[index], overlap );
            }
        }
    }
}
                              ...
                              T *utilization_map)
{
                           ...
                           // as fixed point number
  {
    ...
    unsigned long long int *utilization_buf_map = buf_map;
    ...
    copyScaleArray<<<block_count, thread_count>>>(
        utilization_buf_map, utilization_map, scale_factor, num_bins);
    ...
    copyScaleArray<<<block_count, thread_count>>>(
        utilization_map, utilization_buf_map, T(1.0 / scale_factor), num_bins);
