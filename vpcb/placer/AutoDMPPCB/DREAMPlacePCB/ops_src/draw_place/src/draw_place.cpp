...
/// @param metric_info infomation about current solution 
...
int draw_place_forward(at::Tensor pos, std::string& metric_info,
  ...
  DREAMPLACE_DISPATCH_FLOATING_TYPES(pos, "drawPlaceLauncher", [&] {
    ret = drawPlaceLauncher<scalar_t>(
        ...
        num_nodes, num_nets,
        num_movable_nodes, num_filler_nodes, valid_num_nets, pin2node_map.numel(), xl, yl, xh,
        yh, site_width, row_height, bin_size_x, bin_size_y, metric_info, filename);
  });
