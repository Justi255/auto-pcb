...
class PlaceDrawer {
 public:
  ...
  PlaceDrawer(
      ...
      : m_x(x),
        ...
        m_netpin_start(netpin_start),
        m_flat_netpin(flat_netpin),
        m_num_nodes(num_nodes),
        m_num_nets(num_nets),
        m_num_movable_nodes(num_movable_nodes),
        m_num_filler_nodes(num_filler_nodes),
        m_valid_num_nets(valid_num_nets),
        m_num_pins(num_pins),
        m_xl(xl),
        m_yl(yl),
        m_xh(xh),
  ...
  bool run(std::string const& filename, std::string& metric_info, FileFormat ff) const {
    ...
    switch (ff) {
      ...
      case SVG:
      case PNG:
        flag = writeFig(filename.c_str(), width, height, metric_info, ff);
        break;
      case GDSII:
 ...
 protected:
  ...
  void paintCairo(cairo_surface_t* cs, double width, double height, std::string& metric_info) const {
#if DRAWPLACE == 1
    ...
    cairo_translate(c, -m_xl * ratio[0],
                    -m_yl * ratio[1]);  // translate is additive
    cairo_scale(c, ratio[0], ratio[1]);        // scale is additive
    // x_new =  ratio[0] * x - m_xl * ratio[0]
    // y_new =  ratio[1] * y - m_yl * ratio[1]
    // the same as KICAD
    ...
    {
        int start = m_netpin_start[i];
        int end = m_netpin_start[i + 1];
        ...
        {
            int pin_id = m_flat_netpin[j];
            int node_id = m_pin2node_map[pin_id];
            double node_x = m_x[node_id];
            double node_y = m_y[node_id];
            double xx = node_x + m_pin_offset_x[pin_id];
            x_max = DREAMPLACE_STD_NAMESPACE::max(xx, x_max);
            x_min = DREAMPLACE_STD_NAMESPACE::min(xx, x_min);
            double yy = node_y + m_pin_offset_y[pin_id];
            y_max = DREAMPLACE_STD_NAMESPACE::max(yy, y_max);
            y_min = DREAMPLACE_STD_NAMESPACE::min(yy, y_min);
        }
        cairo_rectangle(c, x_min, y_min, x_max - x_min, y_max - y_min);
        cairo_set_source_rgba(c, 1, 0, 0, std::max((double)m_valid_num_nets/3200, 0.03));
        cairo_fill(c);
        cairo_rectangle(c, x_min, y_min, x_max - x_min, y_max - y_min);
        cairo_set_source_rgba(c, 0.8, 0, 0, std::max((double)m_valid_num_nets/2000, 0.03));
        cairo_stroke(c);
    }

    std::string line;
    auto metric2buf = [&]() {
      size_t pos = metric_info.find('\n');
      if (pos != std::string::npos){
        line = metric_info.substr(0, pos);
        sprintf(buf, "%s", line.c_str());
        metric_info = metric_info.substr(pos + 1);
        return true;
      }
      return false;
    };

    cairo_set_font_size(c, (m_xh - m_xl)/30);
    cairo_set_source_rgb(c, 0, 0, 0);
    double text_x, text_y, text_y_pre = m_yh;
    while(1) {
      if(metric2buf()){
        cairo_text_extents(c, buf, &extents);
        text_x = (m_xh -   m_xl) / 2 - (extents.width  / 2 + extents.x_bearing);
        text_y =  text_y_pre -  (extents.height + extents.y_bearing);
        text_y_pre = text_y_pre - extents.height;
  ...
  bool writeFig(const char* fname, double width, double height, std::string& metric_info,
                FileFormat ff) const {
#if DRAWPLACE == 1
    cairo_surface_t* cs;
    ...
    paintCairo(cs, width, height, metric_info);
