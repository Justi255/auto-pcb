...
bool hannanLegalizeLauncher(LegalizationDB<T> db, std::vector<int>& macros, const std::vector<int>& fixed_macros, int max_iters)
{
    ...
    for (int iter = 0; iter < max_iters; ++iter)
    {
        ...
        for (auto node_id : macros)
        {
            ...
            for (size_t idx = 0; idx < db.num_nodes; idx++)
            {
                dreamplacePrint(kNONE, "%g ", (double)db.x[idx]);
            }
            dreamplacePrint(kNONE, "\n"); 
            for (size_t idx = 0; idx < db.num_nodes; idx++)
            {
                dreamplacePrint(kNONE, "%g ", (double)db.y[idx]);
            }
            dreamplacePrint(kNONE, "\n"); 
            for (auto sx : node_size_x)
            {
                dreamplacePrint(kNONE, "%g ", (double)sx);
            }
            dreamplacePrint(kNONE, "\n"); 
            for (auto sy : node_size_y)
            {
                dreamplacePrint(kNONE, "%g ", (double)sy);
            }
            dreamplacePrint(kNONE, "\n");
        #endif
