...
dct_directory = os.path.join(parent_directory, "dct")
sys.path.append(dct_directory)
sys.path.append(
    os.path.dirname(
        os.path.abspath(__file__)))
import discrete_spectral_transform
import dct2_fft2 as dct
from discrete_spectral_transform import get_exact_expk as precompute_expk

from electric_overflow import ElectricDensityMapFunction as ElectricDensityMapFunction
from electric_overflow import ElectricOverflow as ElectricOverflow
...
class ElectricPotentialFunction(Function):
    ...
    def forward(
        ...
        if rudy_map is not None:
            density_map += rudy_map
        ...
        global plot_count, plot_map_flag_func, img_dir_func
        if plot_map_flag_func:
            path = img_dir_func
            os.makedirs(path, exist_ok=True)
            logger.debug("density_map")
            plot(plot_count, density_map.clone().div(bin_size_x*bin_size_y).cpu().numpy(), path + "%d.density_map" % (plot_count))
            logger.debug("potential_map")
            plot(plot_count, potential_map.clone().cpu().numpy(), path + "%d.potential_map" % (plot_count))
            logger.debug("field_map_x")
            plot(plot_count, ctx.field_map_x.clone().cpu().numpy(), path + "%d.field_map_x" % (plot_count))
            logger.debug("field_map_y")
            plot(plot_count, ctx.field_map_y.clone().cpu().numpy(), path + "%d.field_map_y" % (plot_count))
            plot_count += 1
...
def plot(plot_count, electric_force_map, name):
    """
    electric force heat map
    """

    fig = plt.figure()
    plt.clf()
    plt.pcolor(np.flipud(np.transpose(electric_force_map)))
    fig.tight_layout()
...
class ElectricPotential(ElectricOverflow):
    ...
    def __init__(
        ...
        self.plot_map_flag = plot_map_flag
        self.img_dir = img_dir
        global plot_map_flag_func, img_dir_func
        plot_map_flag_func = plot_map_flag
        img_dir_func =  img_dir
    ...
    def forward(self, pos, rudy_map=None, mode="density"):
        ...
        if self.initial_density_map is None:
            ...
            if self.plot_map_flag:
                path = self.img_dir
                os.makedirs(path, exist_ok=True)
                self.plot(0, self.initial_density_map.clone().div(self.bin_size_x*self.bin_size_y).cpu().numpy(), self.padding, path + 'initial_density_map')
        ...
        if(mode == "density"):
            return ElectricPotentialFunction.apply(
                ...
                self.deterministic_flag, self.sorted_node_map, rudy_map, self.exact_expkM,
