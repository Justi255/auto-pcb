...
class ElectricDensityMapFunction(Function):
    ...
    def forward(
        ...
        # set padding density
        if padding > 0:
            density_map.masked_fill_(padding_mask.bool(),
...
class ElectricOverflow(nn.Module):
    ...
    def reset(self):
        sqrt2 = math.sqrt(2)
        self.node_size_x_clamped = self.node_size_x.clone()
        self.offset_x = torch.zeros_like(self.node_size_x)
        self.node_size_y_clamped = self.node_size_y.clone()
        self.offset_y = torch.zeros_like(self.node_size_y)
        node_areas = self.node_size_x * self.node_size_y
        self.ratio = torch.ones_like(node_areas)
        ...
        if self.target_density < 1 and self.movable_macro_mask is not None:
            ...
            self.ratio[:self.num_movable_nodes].masked_fill_(self.movable_macro_mask.bool(),
                            self.target_density)
    ...
    def plot(self, plot_count, density_map, padding, name):
        """
        ...
        plt.clf()
        plt.pcolor(np.flipud(np.transpose(density_map)))
        fig.tight_layout()
