...
import weighted_average_wirelength_cpp_atomic
import weighted_average_wirelength_cpp_merged
import weighted_average_wirelength_cuda
import weighted_average_wirelength_cuda_atomic
import weighted_average_wirelength_cuda_merged
sys.path.pop()
import pdb
...
class WeightedAverageWirelengthFunction(Function):
    ...
    def backward(ctx, grad_pos):
        ...
        output[:output.numel() // 2].masked_fill_(ctx.pin_mask.bool(), 0.0)
        output[output.numel() // 2:].masked_fill_(ctx.pin_mask.bool(), 0.0)
        if grad_pos.is_cuda:
            torch.cuda.synchronize()
...
class WeightedAverageWirelengthAtomicFunction(Function):
    ...
    def backward(ctx, grad_pos):
        ...
        output = func(grad_pos, ctx.pos, ctx.exp_xy.view([-1]),
                      ...
                      ctx.net_mask, ctx.inv_gamma)
        output[:int(output.numel() // 2)].masked_fill_(ctx.pin_mask.bool(), 0.0)
        output[int(output.numel() // 2):].masked_fill_(ctx.pin_mask.bool(), 0.0)
        if grad_pos.is_cuda:
            torch.cuda.synchronize()
        logger.debug("wirelength backward %.3f ms" %
