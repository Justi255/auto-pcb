...
class Dataset:
    ...
    def update_nodes(self, pos_movable_x, pos_movable_y):
        index_movable = 0
        if self.node_mask is not None:
            for index, node in enumerate(self.sorted_node):
                if not self.node_mask[index]:
                    continue
                else:
                    self.Nodes[node.node_index].x_left   = pos_movable_x[index_movable]
                    self.Nodes[node.node_index].y_bottom = pos_movable_y[index_movable]
                    index_movable           += 1
                    break
        else:
            for node in self.Nodes:
                if node.lock:
                    ...
                    node.x_left   = pos_movable_x[index_movable]
                    node.y_bottom = pos_movable_y[index_movable]
                    index_movable += 1            
