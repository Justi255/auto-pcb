...
public class PlaceControl extends ScopeKeyword {
  ...
  static boolean read_flip_style_rotate_first(IJFlexScanner p_scanner) {
    try {
      ...
      if (next_token != CLOSED_BRACKET) {
        System.out.println("Structure.read_flip_style: closing bracket expected at '" + p_scanner.get_scope_identifier() + "'");
        return false;
      }
      return result;
    } catch (IOException e) {
  ...
  public boolean read_scope(ReadScopeParameter p_par) {
    ...
    for (; ; ) {
      ...
      if (next_token == null) {
        System.out.println("PlaceControl.read_scope: unexpected end of file at '" + p_par.scanner.get_scope_identifier() + "'");
        return false;
      }
      if (next_token == CLOSED_BRACKET) {
        // end of scope
