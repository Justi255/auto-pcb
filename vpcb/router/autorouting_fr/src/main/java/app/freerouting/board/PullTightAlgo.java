...
public abstract class PullTightAlgo {
        ...
        for (SearchTreeObject curr_ob : items) {
          ...
          if (curr_ob instanceof PolylineTrace) {
            ...
            if (OptViaAlgo.opt_via_location(
                this.board, (Via) curr_ob, p_trace_cost_arr, this.min_translate_dist, 10)) {
              something_changed = true;
            }
          }
        }
  ...
  protected boolean is_stop_requested() {
    ...
    if (time_limit_exceeded) {
      ...
      if (this.board == null) {
        System.out.println("PullTightAlgo.is_stop_requested: board is null");
      } else {
        TestLevel test_level = this.board.get_test_level();
        if (test_level == null) {
          System.out.println("PullTightAlgo.is_stop_requested: board test level is null");
