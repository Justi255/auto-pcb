...
public class Simplex extends TileShape implements Serializable {
  ...
  public int dimension() {
    ...
    if (arr.length == 3) {
      ...
      if (side_of_line0 == Side.ON_THE_LEFT) {
        System.out.println("empty Simplex not normalized");//FRLogger.warn("empty Simplex not normalized");
        return -1;
      }
  ...
  public Simplex[] cutout_from(Simplex p_outer_simplex) {
    ...
    for (int inner_corner_no = 0; inner_corner_no < inner_corner_count; ++inner_corner_no) {
      ...
      if (curr_division_lines.length == 2) {
        ...
        if (merge_prev_division_line) ++piece_line_count;
        if (merge_first_division_line) ++piece_line_count;
      ...
      if (merge_next_division_line) ++piece_line_count;
      if (merge_last_curr_division_line) ++piece_line_count;
      if (merge_prev_division_line) ++piece_line_count;
      if (merge_first_division_line) ++piece_line_count;
      Line[] piece_lines = new Line[piece_line_count];
  ...
  private Line[] calc_division_lines(int p_inner_corner_no, Simplex p_outer_simplex) {
    ...
    if (p_inner_corner_no != 0) prev_inner_line = this.arr[p_inner_corner_no - 1];
    ...
    if (intersection.x >= Integer.MAX_VALUE) {
      System.out.println("Simplex.calc_division_lines: intersection expected");
      return null;
    }
        ...
        if (second_division_necessary) {
          ...
          while (!second_projection_visible) {
            ...
            curr_second_projection_dir =
                (IntDirection)
                    inner_corner.perpendicular_direction(p_outer_simplex.arr[tmp_outer_line_no]);
          ...
          curr_distance +=
              Math.abs(
                  p_outer_simplex.arr[tmp_outer_line_no].signed_distance(inner_corner.to_float()));
        }
        if (curr_distance < min_distance) {
          min_distance = curr_distance;
