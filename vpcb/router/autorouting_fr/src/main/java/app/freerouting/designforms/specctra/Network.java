...
public class Network extends ScopeKeyword {
  ...
  private static boolean read_net_pins(IJFlexScanner p_scanner, Collection<Net.Pin> p_pin_list) {
    ...
    if (next_token == null) {
      System.out.println("Network.read_net_pins: unexpected end of file at '" + p_scanner.get_scope_identifier() + "'");
      return false;
    }
    if (next_token != CLOSED_BRACKET) {
      // not end of scope
      System.out.println("Network.read_net_pins: expected closed bracket is missing at '" + p_scanner.get_scope_identifier() + "'");
    }
  ...
  static ViaInfo read_via_info(
    ...
    try {
      ...
      if (!(next_token instanceof String)) {
        System.out.println("Network.read_via_info: string expected at '" + p_scanner.get_scope_identifier() + "'");
        return null;
      }
      ...
      if (via_padstack == null) {
        ...
        if (via_padstack == null) {
          System.out.println("Network.read_via_info: padstack not found at '" + p_scanner.get_scope_identifier() + "'");
          return null;
        }
      ...
      if (next_token != Keyword.CLOSED_BRACKET) {
        if (next_token != Keyword.ATTACH) {
          System.out.println("Network.read_via_info: Keyword.ATTACH expected at '" + p_scanner.get_scope_identifier() + "'");
          return null;
        }
        attach_allowed = true;
        next_token = p_scanner.next_token();
        if (next_token != Keyword.CLOSED_BRACKET) {
          System.out.println("Network.read_via_info: closing bracket expected at '" + p_scanner.get_scope_identifier() + "'");
          return null;
        }
      }
      return new ViaInfo(
  ...
  static Collection<String> read_via_rule(
    ...
    try {
      ...
      for (; ; ) {
        ...
        if (!(next_token instanceof String)) {
          System.out.println("Network.read_via_rule: string expected at '" + p_scanner.get_scope_identifier() + "'");
          return null;
        }
  ...
  static void insert_net_class(
    ...
    if (p_class.trace_clearance_class != null) {
      ...
      if (trace_clearance_class >= 0) {
        ...
        System.out.println("Network.insert_net_class: clearance class not found at '" + board_net_class.get_name() + "'");
      }
    }
    if (p_class.via_rule != null) {
      ...
      if (via_rule != null) {
        ...
        System.out.println("Network.insert_net_class: via rule not found at '" + board_net_class.get_name() + "'");
      }
    }
    ...
    for (Rule curr_rule : p_class.rules) {
      if (curr_rule instanceof Rule.WidthRule) {
        ...
        System.out.println("Network.insert_net_class: rule type not yet implemented at '" + board_net_class.get_name() + "'");
      }
    }
    ...
    for (Rule.LayerRule curr_layer_rule : p_class.layer_rules) {
      for (String curr_layer_name : curr_layer_rule.layer_names) {
        ...
        if (layer_no < 0) {
          System.out.println("Network.insert_net_class: layer not found at '" + board_net_class.get_name() + "'");
          continue;
        }
        for (Rule curr_rule : curr_layer_rule.rules) {
          if (curr_rule instanceof Rule.WidthRule) {
            ...
            System.out.println("Network.insert_net_class: layer rule type not yet implemented at '" + board_net_class.get_name() + "'");
          }
        }
      }
    }
        ...
        if (first_class == null) {
          ...
          while (it2.hasNext()) {
            ...
            if (second_class == null) {
              System.out.println("Network.insert_class_pairs: second class not found");//FRLogger.warn("Network.insert_class_pairs: second class not found");
            } else {
  ...
  private static void insert_class_pair_info(
    ...
    for (Rule.LayerRule curr_layer_rule : p_class_class.layer_rules) {
      for (String curr_layer_name : curr_layer_rule.layer_names) {
        int layer_no = p_board.layer_structure.get_no(curr_layer_name);
        if (layer_no < 0) {
          System.out.println("Network.insert_class_pair_info: layer not found at '" + curr_layer_name + "'");
          continue;
        }
        for (Rule curr_rule : curr_layer_rule.rules) {
          if (curr_rule instanceof Rule.ClearanceRule) {
  ...
  private static int get_clearance_class(
    ...
    if (net_class_no < 0 || result < 0) {
      System.out.println("Network.get_clearance_class: clearance class not found at '" + net_class_name + "'");
      return result;
    }
  ...
  private static boolean insert_logical_parts(ReadScopeParameter p_par) {
    ...
    for (PartLibrary.LogicalPart next_part : p_par.logical_parts) {
      ...
      for (PartLibrary.PartPin curr_part_pin : next_part.part_pins) {
        ...
        if (pin_no < 0) {
          System.out.println("Network.insert_logical_parts: package pin not found at '" + curr_part_pin.pin_name + "'");
          return false;
        }
    ...
    for (PartLibrary.LogicalPartMapping next_mapping : p_par.logical_part_mappings) {
          ...
          routing_board.library.logical_parts.get(next_mapping.name);
      {
        if (curr_logical_part == null) {
          System.out.println("Network.insert_logical_parts: logical part not found at '" + next_mapping.name + "'");
        }
      }
      for (String curr_cmp_name : next_mapping.components) {
        ...
        if (curr_component != null) {
          ...
          System.out.println("Network.insert_logical_parts: board component not found at '" + curr_cmp_name + "'");
        }
      }
    }
    return true;
    ...
    for (PartLibrary.LogicalPartMapping curr_mapping : p_logical_part_mappings) {
      if (curr_mapping.name.equals(p_part_name)) {
        if (curr_mapping.components.isEmpty()) {
          System.out.println("Network.search_lib_package: component list empty at '" + p_part_name + "'");
          return null;
        }
        ...
        if (curr_component == null) {
          System.out.println("Network.search_lib_package: component not found at '" + component_name + "'");
          return null;
        }
        return curr_component.get_package();
      }
    }
    System.out.println("Network.search_lib_package: library package '" + p_part_name + "' not found");
    return null;
  }
  ...
  private static void insert_component(
    ...
    if (curr_front_package == null || curr_back_package == null) {
      System.out.println("Network.insert_component: component package not found at '" + p_par.scanner.get_scope_identifier() + "'");
      return;
    }
    ...
    for (int i = 0; i < curr_package.pin_count(); ++i) {
      ...
      if (curr_padstack == null) {
        System.out.println("Network.insert_component: pin padstack not found at '" + p_par.scanner.get_scope_identifier() + "'");
        return;
      }
      ...
      for (Net curr_pin_net : pin_nets) {
        ...
        if (curr_board_net == null) {
          System.out.println("Network.insert_component: board net not found at '" + p_par.scanner.get_scope_identifier() + "'");
        } else {
    ...
    for (int k = 0; k <= 2; ++k) {
      ...
      for (int i = 0; i < keepout_arr.length; ++i) {
        ...
        if (layer >= routing_board.get_layer_count()) {
          System.out.println("Network.insert_component: keepout layer is to big at '" + p_par.scanner.get_scope_identifier() + "'");
          continue;
        }
  ...
  public boolean read_scope(ReadScopeParameter p_par) {
    ...
    for (; ; ) {
      ...
      if (next_token == null) {
        System.out.println("Network.read_scope: unexpected end of file at '" + p_par.scanner.get_scope_identifier() + "'");
        return false;
      }
      if (next_token == CLOSED_BRACKET) {
        // end of scope
    ...
    if (p_par.via_padstack_names != null) {
      ...
      for (int i = 0; i < via_padstacks.length; ++i) {
        ...
        if (curr_padstack != null) {
          ...
          System.out.println("Library.read_scope: via padstack with name '" + curr_padstack_name + " not found at '" + p_par.scanner.get_scope_identifier() + "'");
        }
      }
  ...
  private boolean read_net_scope(
    ...
    if (!scope_is_empty) {
      for (; ; ) {
        ...
        if (next_token == null) {
          System.out.println("Network.read_net_scope: unexpected end of file at '" + p_scanner.get_scope_identifier() + "'");
          return false;
        }
        if (next_token == CLOSED_BRACKET) {
          // end of scope
        ...
        if (prev_token == OPEN_BRACKET) {
          if (next_token == Keyword.PINS) {
            ...
            System.out.println("Network.read_net_scope: layer_rule not yet implemented at '" + p_scanner.get_scope_identifier() + "'");
    ...
    for (Collection<Net.Pin> curr_pin_list : subnet_pin_lists) {
      ...
      if (curr_subnet == null) {
        System.out.println("Network.read_net_scope: net not found in netlist at '" + p_scanner.get_scope_identifier() + "'");
        return false;
      }
      ...
      if (!net_rules.isEmpty()) {
        ...
        if (board_net == null) {
          System.out.println("Network.read_net_scope: board net not found at '" + p_scanner.get_scope_identifier() + "'");
          return false;
        }
        for (Rule curr_ob : net_rules) {
          if (curr_ob instanceof Rule.WidthRule) {
            ...
            System.out.println("Network.read_net_scope: Rule not yet implemented at '" + p_scanner.get_scope_identifier() + "'");
          }
        }
      }
      ++subnet_number;
