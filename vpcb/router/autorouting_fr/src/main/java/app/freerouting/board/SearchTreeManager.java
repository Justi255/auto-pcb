...
public class SearchTreeManager {
  ...
  public ShapeSearchTree get_autoroute_tree(int p_clearance_class_no) {
    ...
    if (fast_algorithm
        && this.board.rules.get_trace_angle_restriction() == AngleRestriction.NINETY_DEGREE) {
      curr_autoroute_tree = new ShapeSearchTree90Degree(this.board, p_clearance_class_no);
    } else if (fast_algorithm
        && this.board.rules.get_trace_angle_restriction() == AngleRestriction.FORTYFIVE_DEGREE) {
  ...
  private void remove_all_board_items() {
    if (this.board == null) {
      System.out.println("SearchtreeManager.remove_all_board_items: app.freerouting.board is null");
      return;
    }
    Iterator<UndoableObjects.UndoableObjectNode> it = this.board.item_list.start_read_object();
    for (; ; ) {
  ...
  private void insert_all_board_items() {
    if (this.board == null) {
      System.out.println("SearchtreeManager.insert_all_board_items: app.freerouting.board is null");
      return;
    }
    Iterator<UndoableObjects.UndoableObjectNode> it = this.board.item_list.start_read_object();
    for (; ; ) {
