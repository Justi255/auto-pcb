...
public class Library extends ScopeKeyword {
  ...
  static boolean read_padstack_scope(
    ...
    try {
      ...
      if (next_token instanceof String) {
        ...
        System.out.println("Library.read_padstack_scope: unexpected padstack identifier at '" + p_scanner.get_scope_identifier() + "'");
        return false;
      }
        ...
        if (prev_token == Keyword.OPEN_BRACKET) {
          if (next_token == Keyword.SHAPE) {
            ...
            if (curr_next_token != Keyword.CLOSED_BRACKET) {
              System.out.println("Library.read_padstack_scope: closing bracket expected at '" + p_scanner.get_scope_identifier() + "'");
              return false;
            }
    ...
    for (Shape pad_shape : shape_list) {
      ...
      if (curr_shape instanceof ConvexShape) {
        ...
        if (convex_shapes.length != 1) {
          System.out.println("Library.read_padstack_scope: convex shape expected at '" + p_scanner.get_scope_identifier() + "'");
        }
      ...
      if (pad_shape.layer == Layer.PCB || pad_shape.layer == Layer.SIGNAL) {
        ...
        if (shape_layer < 0 || shape_layer >= padstack_shapes.length) {
          System.out.println("Library.read_padstack_scope: layer number found at '" + p_scanner.get_scope_identifier() + "'");
          return false;
        }
  ...
  public boolean read_scope(ReadScopeParameter p_par) {
    ...
    for (; ; ) {
      ...
      if (next_token == null) {
        System.out.println("Library.read_scope: unexpected end of file at '" + p_par.scanner.get_scope_identifier() + "'");
        return false;
      }
    ...
    for (Package curr_package : package_list) {
      ...
      for (int i = 0; i < pin_arr.length; ++i) {
        ...
        if (board_padstack == null) {
          System.out.println("Library.read_scope: board padstack not found at '" + p_par.scanner.get_scope_identifier() + "'");
          return false;
        }
        pin_arr[i] =
      ...
      for (int i = 0; i < outline_arr.length; ++i) {
        ...
        if (curr_shape != null) {
          ...
          System.out.println("Library.read_scope: outline shape is null at '" + p_par.scanner.get_scope_identifier() + "'");
        }
      }
