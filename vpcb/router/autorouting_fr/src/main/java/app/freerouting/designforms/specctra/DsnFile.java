...
public class DsnFile {
  ...
  public static ReadResult read(
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      if (!keyword_ok) {
        System.out.println("DsnFile.read: the input file is not in a Specctra DSN file format. It must be a text file starting with the '(pcb' character array.");
        return ReadResult.ERROR;
      }
    }
  ...
  static boolean read_on_off_scope(IJFlexScanner p_scanner) {
    try {
      ...
      if (next_token == Keyword.ON) {
        ...
        System.out.println("DsnFile.read_boolean: Keyword.OFF expected at '" + p_scanner.get_scope_identifier() + "'");
      }
  ...
  static int read_integer_scope(IJFlexScanner p_scanner) {
    try {
      ...
      if (next_token instanceof Integer) {
        ...
        System.out.println("DsnFile.read_integer_scope: number expected at '" + p_scanner.get_scope_identifier() + "'");
        return 0;
      }
      next_token = p_scanner.next_token();
      if (next_token != Keyword.CLOSED_BRACKET) {
        System.out.println("DsnFile.read_integer_scope: closing bracket expected at '" + p_scanner.get_scope_identifier() + "'");
        return 0;
      }
      return value;
    } catch (IOException e) {
  ...
  static double read_float_scope(IJFlexScanner p_scanner) {
    try {
      ...
      if (next_token instanceof Double) {
        ...
        System.out.println("DsnFile.read_float_scope: number expected at '" + p_scanner.get_scope_identifier() + "'");
        return 0;
      }
      next_token = p_scanner.next_token();
      if (next_token != Keyword.CLOSED_BRACKET) {
        System.out.println("DsnFile.read_float_scope: closing bracket expected at '" + p_scanner.get_scope_identifier() + "'");
        return 0;
      }
      return value;
    } catch (IOException e) {
  ...
  public static String read_string_scope(IJFlexScanner p_scanner) {
    try {
      ...
      Object next_token = p_scanner.next_token();
      if (next_token != Keyword.CLOSED_BRACKET) {
        System.out.println("DsnFile.read_string_scope: closing bracket expected at '" + p_scanner.get_scope_identifier() + "'");
      }
      return result;
    } catch (IOException e) {
