...
public class Parser extends ScopeKeyword {
  ...
  private static SpecctraParserInfo.WriteResolution read_write_solution(ReadScopeParameter p_par) {
    try {
      ...
      if (!(next_token instanceof String)) {
        System.out.println("Parser.read_write_solution: string expected at '" + p_par.scanner.get_scope_identifier() + "'");
        return null;
      }
      String resolution_string = (String) next_token;
      next_token = p_par.scanner.next_token();
      if (!(next_token instanceof Integer)) {
        System.out.println("Parser.read_write_solution: integer expected expected at '" + p_par.scanner.get_scope_identifier() + "'");
        return null;
      }
      int resolution_value = (Integer) next_token;
      next_token = p_par.scanner.next_token();
      if (next_token != CLOSED_BRACKET) {
        System.out.println("Parser.read_write_solution: closing_bracket expected at '" + p_par.scanner.get_scope_identifier() + "'");
        return null;
      }
  ...
  private static String[] read_constant(ReadScopeParameter p_par) {
    try {
      ...
      Object next_token = p_par.scanner.next_token();
      if (!(next_token instanceof String)) {
        System.out.println("Parser.read_constant: string expected at '" + p_par.scanner.get_scope_identifier() + "'");
        return null;
      }
      ...
      next_token = p_par.scanner.next_token();
      if (next_token != CLOSED_BRACKET) {
        System.out.println("Parser.read_constant: closing_bracket expected at '" + p_par.scanner.get_scope_identifier() + "'");
        return null;
      }
      return result;
    } catch (IOException e) {
  ...
  private static String read_quote_char(IJFlexScanner p_scanner) {
    try {
      ...
      if (!(next_token instanceof String)) {
        System.out.println("Parser.read_quote_char: string expected at '" + p_scanner.get_scope_identifier() + "'");
        return null;
      }
      String result = (String) next_token;
      next_token = p_scanner.next_token();
      if (next_token != CLOSED_BRACKET) {
        System.out.println("Parser.read_quote_char: closing bracket expected at '" + p_scanner.get_scope_identifier() + "'");
        return null;
      }
      return result;
    } catch (IOException e) {
  ...
  public boolean read_scope(ReadScopeParameter p_par) {
    ...
    for (; ; ) {
      ...
      try {
        ...
        System.out.println("Parser.read_scope: IO error scanning file at '" + p_par.scanner.get_scope_identifier() + "'");
        return false;
      }
      if (next_token == null) {
        System.out.println("Parser.read_scope: unexpected end of file at '" + p_par.scanner.get_scope_identifier() + "'");
        return false;
      }
      if (next_token == CLOSED_BRACKET) {
        // end of scope
