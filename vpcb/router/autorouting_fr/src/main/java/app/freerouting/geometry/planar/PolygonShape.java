...
public class PolygonShape extends PolylineShape {
  ...
  public boolean intersects(Circle p_circle) {
    ...
    for (int i = 0; i < convex_pieces.length; ++i) {
      if (convex_pieces[i].intersects(p_circle)) return true;
    }
    return false;
  }
  ...
  public boolean intersects(Simplex p_simplex) {
    TileShape[] convex_pieces = split_to_convex();
    for (int i = 0; i < convex_pieces.length; ++i) {
      if (convex_pieces[i].intersects(p_simplex)) return true;
    }
    return false;
  }
  ...
  public boolean intersects(IntOctagon p_oct) {
    TileShape[] convex_pieces = split_to_convex();
    for (int i = 0; i < convex_pieces.length; ++i) {
      if (convex_pieces[i].intersects(p_oct)) return true;
    }
    return false;
  }
  ...
  public boolean intersects(IntBox p_box) {
    TileShape[] convex_pieces = split_to_convex();
    for (int i = 0; i < convex_pieces.length; ++i) {
      if (convex_pieces[i].intersects(p_box)) return true;
    }
    return false;
  }
  ...
  public boolean contains(FloatPoint p_point) {
    TileShape[] convex_pieces = split_to_convex();
    for (int i = 0; i < convex_pieces.length; ++i) {
      if (convex_pieces[i].contains(p_point)) return true;
    }
    return false;
  }
  ...
  public boolean is_outside(Point p_point) {
    TileShape[] convex_pieces = split_to_convex();
    for (int i = 0; i < convex_pieces.length; ++i) {
      if (!convex_pieces[i].is_outside(p_point)) return false;
    }
    return true;
  }
  ...
  public boolean is_convex() {
    ...
    for (int ind = 0; ind < corners.length; ++ind) {
      if (next_point.side_of(prev_point, curr_point) == Side.ON_THE_RIGHT) return false;
      prev_point = curr_point;
      curr_point = next_point;
      if (ind == corners.length - 2) next_point = corners[0];
      else next_point = corners[ind + 2];
    }
    ...
    for (int ind2 = 2; ind2 < corners.length; ++ind2) {
      ...
      if (last_det <= 0 && curr_det > 0) return false;
      last_det = curr_det;
    }
  ...
  private Collection<PolygonShape> split_to_convex_recu() {
    ...
    for (int i = 0; i < corners.length; ++i) {
      if (start_corner_no < corners.length - 1) next_corner = corners[start_corner_no + 1];
      else next_corner = corners[0];
    ...
    if (corner_count < 0) corner_count += corners.length;
    ++corner_count;
  ...
  private class DivisionPoint {
    ...
    DivisionPoint(int p_concave_corner_no) {
      ...
      else before_concave_corner = corners[corners.length - 1].to_float();

      FloatPoint after_concave_corner;

      if (p_concave_corner_no == corners.length - 1) after_concave_corner = corners[0].to_float();
      else after_concave_corner = corners[p_concave_corner_no + 1].to_float();
      ...
      else corner_before_curr_projection = corners[corners.length - 1];
