...
public class ShapeSearchTree extends MinAreaTree {
  ...
  public void overlapping_tree_entries(
    ...
    if (p_tree_entries == null) {
      System.out.println("ShapeSearchTree.overlaps: p_obstacle_entries is null");
      return;
    }
    RegularTileShape bounds = p_shape.bounding_shape(bounding_directions);
    if (bounds == null) {
      System.out.println("ShapeSearchTree.overlaps: p_shape not bounded");
      return;
    }
  ...
  void overlapping_tree_entries_with_clearance(
    ...
    if (p_obstacle_entries == null) {
      System.out.println("ShapeSearchTree.overlaps_with_clearance: p_obstacle_entries is null");
      return;
    }
    ClearanceMatrix cl_matrix = board.rules.clearance_matrix;
    RegularTileShape bounds = p_shape.bounding_shape(bounding_directions);
    if (bounds == null) {
      System.out.println("ShapeSearchTree.overlaps_with_clearance: p_shape is not bounded");
    ...
    for (EntrySortedByClearance tmp_entry : sorted_items) {
      ...
      if (curr_offset_shape.intersects(tmp_offset_shape)) {
        p_obstacle_entries.add(
            new TreeEntry(tmp_entry.leaf.object, tmp_entry.leaf.shape_index_in_object));
      }
    }
  }
  ...
  protected Collection<IncompleteFreeSpaceExpansionRoom> divide_large_room(
    ...
    if (2 * room_bounding_box.height() <= p_board_bounding_box.height()
